            ...
            for (Schedule lesson : schedule) {
                ...
                if (timeslot == 0) {
                    if (lesson.getCancelled()) {
                        String string;
                        if (!lesson.getType().equals("Les"))
                            string = lesson.getSubjectAndGroup(sp) + " (" + lesson.getType() + ") " + lesson.getLocation() + "\n";
                        else
                            string = lesson.getSubjectAndGroup(sp) + " " + lesson.getLocation() + "\n";
                        strikethroughStartIndex.add(widgetText.length());
                        strikethroughEndIndex.add(widgetText.length() + string.length());
                        widgetText += string;
                    } else {
                        if (!lesson.getType().equals("Les"))
                            widgetText += lesson.getSubjectAndGroup(sp) + " (" + lesson.getType() + ") " + lesson.getLocation() + "\n";
                        else
                            widgetText += lesson.getSubjectAndGroup(sp) + " " + lesson.getLocation() + "\n";
                    }
                } else {
                    if (lesson.getCancelled()) {
                        String string;
                        if (!lesson.getType().equals("Les"))
                            string = String.valueOf(timeslot) + ". " + lesson.getSubjectAndGroup(sp) + " (" + lesson.getType() + ") " + lesson.getLocation() + "\n";
                        else
                            string = String.valueOf(timeslot) + ". " + lesson.getSubjectAndGroup(sp) + " " + lesson.getLocation() + "\n";
                        strikethroughStartIndex.add(widgetText.length());
                        strikethroughEndIndex.add(widgetText.length() + string.length());
                        widgetText += string;
                    } else {
                        if (!lesson.getType().equals("Les"))
                            widgetText += String.valueOf(timeslot) + ". " + lesson.getSubjectAndGroup(sp) + " (" + lesson.getType() + ") " + lesson.getLocation() + "\n";
                        else
                            widgetText += String.valueOf(timeslot) + ". " + lesson.getSubjectAndGroup(sp) + " " + lesson.getLocation() + "\n";
                    }
                }
            }
